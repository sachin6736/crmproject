...
const LeadQuotationPreviewModal = ({
  isOpen,
  onClose,
  emailContent,
  leadId,
  ...
  const handleConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/Lead/leadquatation/${leadId}`,
        {
          method: "POST",
          credentials: "include",
      ...
      toast.success(data.message || "Quotation sent successfully");
      onClose(); // Close the modal
      onQuotationSent(); // Trigger refresh in parent component
    } catch (error) {
...
export default LeadQuotationPreviewModal;