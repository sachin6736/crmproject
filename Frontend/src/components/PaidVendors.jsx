...
const PaidVendors = () => {
    ...
    const fetchPaidVendors = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/Order/paidvendorlist?page=${page}&search=${encodeURIComponent(
            search
          )}`,
          {
            method: 'GET',
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.message || 'Failed to fetch paid vendors');
        }
        const data = await response.json();
        ...
        setError(err.message || 'Failed to fetch paid vendors. Please check your network connection.');
  ...
  const handleNoteSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/Order/paidvendor/${selectedVendorId}/notes`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          ...
          <div className="mb-6 max-w-lg mx-auto relative">
            <div className="flex items-center">
              <input
                ...
                aria-label="Search paid vendors"
          ...
          {error ? (
            ...
            <div className="overflow-x-auto shadow-lg rounded-lg">
              ...
              >
                ...
                <tbody>
                  {paidVendors.map((vendorData, index) => (
                    <tr
                      key={vendorData._id}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm">
                        {vendorData.paidAt ? new Date(vendorData.paidAt).toLocaleString() : 'N/A'}
                      </td>
                      <td
