...
// Note: Removed unused ProcurementOrderDetailsModal import; use CustomerRelationsOrderDetailsModal if it exists

const CustomerRelationsDashboard = () => {
  const { theme } = useTheme();
  const [totalOrders, setTotalOrders] = useState(0);
  const [deliveredCount, setDeliveredCount] = useState(0);
  const [shipOutTotalAmount, setShipOutTotalAmount] = useState(0);
  const [orders, setOrders] = useState([]);
  const [shipOutOrders, setShipOutOrders] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchMainData = async () => {
      try {
        const query = [`customerRelationsPerson=${user._id}`];
        ...
        const [
          ...
          fetch(
            `${import.meta.env.VITE_API_URL}/CustomerRelations/getCustomerRelationsOrders${queryString}`,
            {
              method: "GET",
              credentials: "include",
            }
          ...
          fetch(
            `${import.meta.env.VITE_API_URL}/CustomerRelations/getCustomerRelationsOrderAmountTotals${queryString}`,
            {
              method: "GET",
              credentials: "include",
            }
          ...
          fetch(
            `${import.meta.env.VITE_API_URL}/CustomerRelations/getCustomerRelationsDeliveredMetrics${queryString}`,
            {
              method: "GET",
              credentials: "include",
            }
        ...
        if (!ordersRes.ok)
          errors.push(
            `Failed to fetch customer relations orders: ${ordersRes.status}`
          );
        ...
        setShipOutTotalAmount(shipOutTotalAmountData);
        setOrders(ordersData || []);
        setShipOutOrders(shipOutOrdersData || []);
    ...
    const fetchComparisonData = async () => {
      try {
        ...
        const statusComparisonRes = await fetch(
          `${import.meta.env.VITE_API_URL}/CustomerRelations/getCustomerRelationsOrderStatusComparison${queryString}`,
          {
            method: "GET",
            credentials: "include",
          }
      ...
      <div className={`${loading || chartLoading ? "blur-[1px]" : ""}`}>
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              ...
              {
                title: "Today's Total",
                value: `$${(deliveredMetrics.today.revenue || 0).toFixed(2)}`,
                viewMode: "todayTotal",
                icon: <DollarSign className="w-5 h-5 text-blue-500" />,
              },
              {
                title: "Ship Out Total Amount",
                value: `$${(shipOutTotalAmount || 0).toFixed(2)}`,
                viewMode: "shipOutTotalAmount",
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 overflow-x-auto">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">
                Recent Customer Relations Orders
              </h2>
        ...
        <AnimatePresence>
          {showOrderDetailsModal && (
            <CustomerRelationsOrderDetailsModal
              ...
              shipOutOrders={shipOutOrders}
              shipOutTotalAmount={shipOutTotalAmount}
              viewMode={viewMode}
            />
          )}
        </AnimatePresence>
