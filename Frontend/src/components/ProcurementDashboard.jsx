...
import { Trash2, TrendingUp, DollarSign, Send, Truck, CheckCircle, BarChart2 } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchProcurementData = async () => {
      ...
      try {
        ...
        const [ordersRes, statusComparisonRes, orderAmountTotalsRes, deliveredMetricsRes] = await Promise.all([
          ...
          fetch(`http://localhost:3000/Procurement/getProcurementDeliveredMetrics${queryString}`, {
            method: 'GET',
            credentials: 'include',
          }),
        ]);
    ...
    <div className="w-full min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-900 dark:to-gray-800 text-gray-900 dark:text-gray-100 transition-colors duration-300">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-8">
          {[
            { title: 'PO Sent', value: poSentCount, viewMode: 'poSent', icon: <Send className="w-5 h-5 text-orange-500" /> },
            { title: 'Delivered', value: deliveredCount, viewMode: 'delivered', icon: <Truck className="w-5 h-5 text-lime-500" /> },
            { title: 'Total Orders', value: totalOrders, viewMode: 'totalOrders', icon: <CheckCircle className="w-5 h-5 text-green-500" /> },
            { title: "Today's Total", value: `$${(orderAmountTotals.today || 0).toFixed(2)}`, viewMode: 'todayTotal', icon: <DollarSign className="w-5 h-5 text-blue-500" /> },
            { title: 'PO Sent Total Amount', value: `$${(poSentTotalAmount || 0).toFixed(2)}`, viewMode: 'poSentTotalAmount', icon: <DollarSign className="w-5 h-5 text-purple-500" /> },
