  ...
  return (
    <div className="w-full min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      <div className="flex flex-wrap gap-6 p-3 px-4 sm:px-20">
        {['POSent', 'Delivered', 'TotalOrders', 'TodayTotal', 'POSentTotalAmount'].map((key) => (
          <div
            key={key}
            ...
            onClick={() => {
              setViewMode(key === 'POSent' ? 'poSent' : 'full');
              setShowOrderDetailsModal(true);
            }}
          >
            <h3 className="text-gray-500 dark:text-gray-400 text-lg flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-blue-500" />
              {key === 'POSent' ? 'PO Sent' : key === 'TotalOrders' ? 'Total Orders' : key === 'TodayTotal' ? "Today's Total" : key === 'POSentTotalAmount' ? 'PO Sent Total Amount' : 'Delivered'}
            </h3>
            <span className="text-4xl font-bold text-blue-600 dark:text-blue-400">
              {key === 'POSent' ? poSentCount : key === 'TotalOrders' ? totalOrders : key === 'TodayTotal' ? `$${(orderAmountTotals.today || 0).toFixed(2)}` : key === 'POSentTotalAmount' ? `$${(poSentTotalAmount || 0).toFixed(2)}` : deliveredCount}
            </span>
          </div>
        ))}
      </div>
