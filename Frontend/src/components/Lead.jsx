...
import { Triangle, Save, Edit, Trash2, ChevronDown, Check } from "lucide-react";
...
const Lead = () => {
  ...
  const handleDownload = () => {
    const { notes, importantDates, __v, _id, ...rest } = singleLead;
  
    const formattedNotes = (notes || [])
      .map(
        (note, i) =>
          `Note ${i + 1}: ${note.text} on ${new Date(
            note.createdAt
          ).toLocaleString()})`
      )
      .join(" | ");
  
    const formattedDates = (importantDates || [])
      .map(
        (date, i) =>
          `Date ${i + 1}: ${date.title} on ${new Date(date.date).toLocaleDateString()}`
      )
      .join(" | ");
    ...
    exportToExcel(
      exportData,
      `${singleLead.clientName || "lead"}_details.xlsx`
    );
  };
                ...
                >
                  ...
                  <div className="space-x-2">
                    <input
                      type="text"
                      placeholder="$"
                      className="w-28 h-10 bg-white border-none rounded-md"
                    />
                    <button>
                      <Check size={16} />
                    </button>
